			...
			if (map[mapReference] && polylines) {
				...
				for (var polyline of polylines) {
					...
					for (var polylineLocationIndex = 0; polylineLocationIndex < polylineLocations.length && polylineLocations[polylineLocationIndex][0] && polylineLocations[polylineLocationIndex][1]; polylineLocationIndex++) {
						var entity = new Microsoft.Maps.Location(polylineLocations[polylineLocationIndex][0], polylineLocations[polylineLocationIndex][1]);
						entity.metadata = polyline.metadata; // put metadata into entity
						polylineLocationsAsMapLocations.push(entity);
					}
					...
					if (polylineLocationsAsMapLocations.length !== 0) {
						...
						polylineObject.metadata = polyline.metadata; // put metadata into entity
